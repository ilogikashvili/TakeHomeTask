...
- Pre-staging contract cleanup (2026-09-11): 62 unit and 68 PostgreSQL E2E tests, backend lint/build and frontend build passed again. Ten additional deployment-role tests passed against fresh local PostgreSQL 18.6 with SCRAM-authenticated application logins; exact ACLs, normal runtime operations and prohibited operations were verified. All three migrations, validation and status passed. The six Playwright tests remain previously recorded evidence, not a new run. [Deployment contract and evidence](deploy/DEPLOYMENT_CONTRACT.md).
- Historical pre-staging finding: PostgreSQL certificate/TLS configuration was missing. **Resolved locally in the subsequent TLS gate (2026-09-11):** Compose enables TLS and rejects plaintext TCP; 13 TLS/role tests, 68 TLS PostgreSQL E2E tests, 62 unit tests, lint/builds, Prisma validation/migrations/status and zero-vulnerability dependency audits passed. Production validation and privilege grants were not weakened. Real staging certificate installation/renewal ownership, remote CI/container proof and other environment gates remain open. Docker Desktop and real staging were untouched; D2 remains open. [TLS evidence](deploy/DEPLOYMENT_CONTRACT.md#tls-gate-evidence--2026-09-11).
