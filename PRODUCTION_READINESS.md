...
- Current recorded baseline (REMAINING_WORK.md): 62 passing backend unit tests and 68 passing PostgreSQL E2E tests; Prisma validation/generation, migration deployment/status, build and lint pass.
- Current recorded frontend baseline: production build and 6 browser tests pass. The suite covers owner/admin flows, optimistic conflict handling, vendor clarification, logout/expired sessions, assistant links, reminders access and mobile rendering.
- Historical review snapshot: 61 unit tests, 49 PostgreSQL E2E tests and 4 browser tests. These earlier counts are retained as history, not the current baseline.
- Pre-staging cleanup, 2026-09-11: the 62 unit / 68 PostgreSQL E2E baseline, both builds and backend lint passed again. Ten additional deployment-role tests passed on a fresh local PostgreSQL 18.6 cluster, using the actual initializer/grants and SCRAM-authenticated application logins. Prisma validation, all three migrations and migration status passed. The six-browser-test count remains prior local evidence; Playwright was not rerun because no frontend deployment configuration changed. See [the deployment contract](deploy/DEPLOYMENT_CONTRACT.md) for scope and limitations.
- Subsequent TLS gate, 2026-09-11: **PostgreSQL TLS configuration is resolved locally.** The bundled service now requires TLS/SCRAM, with operator-supplied certificates outside Git. On disposable PostgreSQL 18.6, all **13 TLS/role tests** passed (including the prior ten privilege scenarios), plaintext was rejected for all three logins, untrusted CAs were rejected, migrations/status passed over TLS, and the compiled production application passed startup/readiness/assistant checks. **68 E2E tests passed over TLS**, all **62 unit tests** and requested builds/lint passed, and both dependency audits reported **zero vulnerabilities**. Real certificate provisioning, remote CI/container evidence and staging remain pending; [TLS evidence and diff review](deploy/DEPLOYMENT_CONTRACT.md#tls-gate-evidence--2026-09-11).
...
| P0 | Snapshot consistency is defined and locally verified: ledger page plus aggregates and CSV pagination run in repeatable-read transactions. Serializable conflict guarantees are not claimed. | Production-scale concurrent workload evidence remains open. |
