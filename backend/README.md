...
Read-only execution explicitly opens a read-only PostgreSQL transaction with a five-second statement timeout. Bootstrap separate deployment logins with [01-runtime-roles.sh](scripts/01-runtime-roles.sh), then apply the column allowlist through the existing migration service as described in the [runbook](../deploy/RUNBOOK.md). Set the assistant URL to `ledger_readonly`; it can read only the required LineItem and Vendor columns. The assistant fails closed when the read-only URL is missing.
