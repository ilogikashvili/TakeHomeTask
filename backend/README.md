...
The Gemini adapter follows [Google's structured-output REST documentation](https://ai.google.dev/gemini-api/docs/generate-content/structured-output?hl=en). Its contract and failure handling are tested with mocked responses. Live provider execution has not been verified in this workspace. Final answer wording remains deterministic; model explanations and LangGraph orchestration are still pending. The earlier graph-node files are unused scaffolding.
